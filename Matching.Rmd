...
iraq <- merge(i, rugged_i, all.x = TRUE, all.y = TRUE)
...
rugged_s <- read_csv("Admin2006_Ruggedness_1KM.csv") %>%
  ...
  mutate(province = ADMIN_NAME) %>%
  mutate(country = CNTRY_NAME) %>%
  select(country, province, POP_ADMIN, SQKM, CELL_COUNT, MIN, MAX, RANGE, MEAN, STD, SUM)

s <- merge(territory_s, distance_s, all.x = TRUE, all.y = TRUE)

somalia <- merge(s, rugged_s, all.x = TRUE, all.y = TRUE)

```

```{r}

# Load Data from package caseMatch
library(caseMatch)
data(EU)
## Define matching variables
mvars <- c("socialist","rgdpc","FHc","FHp","trade")
# Define the "leftover" variables in the dataset to not include in the matching
dropvars <- c("countryname", "population", "eu")
...
out2 <- (case.match(data=EU, id.var="countryname", leaveout.vars=dropvars,
                   distance="mahalanobis", case.N=2, 
                   number.of.matches.to.return=10,
                   treatment.var="socialist", max.variance=TRUE))
